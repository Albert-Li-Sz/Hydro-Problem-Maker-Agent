...
Paste a complete statement and run Pi Agent to generate a standard solution, independent oracle, testlib generator and validator, data, and a C++ testlib SPJ when needed. Build the [Docker sandbox](packages/hydro-agent/README.md) first. The platform verifies the project before releasing a Hydro ZIP and a separate source/evidence ZIP. Optional uploaded code can be corrected or replaced automatically. **重制 / 下一题** clears the draft without cancelling saved tasks; history supports continuation, downloading and deleting records.

For existing Pi credentials, `HYDRO_ENABLE_AGENT=1` enables ambient credentials; `HYDRO_MODEL_PROVIDER` and `HYDRO_MODEL_ID` select the model. Runs use a single-concurrency queue with SSE progress and cancellation. Local validation covers ordinary batch problems and testlib SPJ; live Hydro import and judging remain separate checks.
